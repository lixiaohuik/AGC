...
run-vlsi: $(src_files) vlsi-asic
	...
	cd ../../..
	sbt "run -params_$(FIXED)_$(VERILOGTB) --test --genHarness --backend null --debugMem --debug --targetDir build/vlsi/generated-src --testCommand \"build/vlsi/vcs-sim-rtl/AGC\""
...
run-vlsi-vpd: $(src_files) vlsi-asic
	cd build/vlsi/vcs-sim-rtl && make
	cd ../../..
	sbt "run -params_$(FIXED)_$(VERILOGTB) --test --genHarness --backend null --debugMem --debug --targetDir build/vlsi/generated-src --testCommand \"build/vlsi/vcs-sim-rtl/AGC\" +vcdfile=build/vlsi/vcs-sim-rtl/AGC.vcd"


run-vlsi-syn: $(src_files) vlsi-asic
	cd build/vlsi/vcs-sim-gl-syn && make
	cd ../../..
	sbt "run -params_$(FIXED)_$(VERILOGTB) --test --backend null --targetDir build/vlsi/vcs-sim-gl-syn --testCommand \"build/vlsi/vcs-sim-gl-syn/AGC -ucli -do build/vlsi/vcs-sim-gl-syn/+run_from_prjdir.tcl\""

run-vlsi-par: $(src_files) vlsi-asic
	cd build/vlsi/vcs-sim-gl-par && make SHELL=/bin/bash
	cd ../../..
	sbt "run -params_$(FIXED)_$(VERILOGTB) --test --backend null --targetDir build/vlsi/vcs-sim-gl-par --testCommand \"build/vlsi/vcs-sim-gl-par/AGC -ucli -do build/vlsi/vcs-sim-gl-par/+run_from_prjdir.tcl\""
